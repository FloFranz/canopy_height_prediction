...
def print_train_progress(epoch, epochs, batch, batches, loss):
    ...
    text = "\33[2K\r{:2d}/{}: |{}|{}| - Loss: {:.4f}    ".format(epoch, epochs, t_bar, v_bar, loss)
...
def print_final(epoch, epochs, mean_train_loss, mean_val_loss):
    text = "\33[2K\r{:2d}/{}: |{}|{}| - Loss: {:.4f} - {:.4f}    ".format(epoch, epochs, PROGRESS_BAR_TRAIN * "=", PROGRESS_BAR_VALIDATE * "=", mean_train_loss, mean_val_loss)
    sys.stdout.write(text)
    sys.stdout.flush()